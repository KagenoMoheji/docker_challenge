...
import {ParamEventListener} from "~/types/eventListener";
...
export const Header = (): JSX.Element => {
  ...
  useEffect(() => {
    const paramsEventListener: ParamEventListener[] = [
      [
        ["type", "click"],
        ["listener", onClickOutofHumb],
      ],
    ];
    const valsEventListener = paramsEventListener[0].map(r => (r === undefined) ? undefined : r[1]);
    document.addEventListener(...valsEventListener);
    return () => {
      document.removeEventListener(...valsEventListener);
    };
  }, []);
  return (
